...
class GplayUpgradeOwnershipTest : BaseTest() {
    ...
    fun `one-time purchase maps to iap ownership`() {
        val ownership = info(mockPurchase("eu.darken.myperm.iap.upgrade.pro")).toOwnership()
    ...
    fun `owning both products is represented as both`() {
        val ownership = info(
            mockPurchase("eu.darken.myperm.iap.upgrade.pro"),
    ...
    fun `a pending payment sets the pending flag`() {
        pendingInfo(mockPurchase("eu.darken.myperm.iap.upgrade.pro")).toPendingFlag().shouldBeTrue()
        pendingInfo(mockPurchase("upgrade.pro")).toPendingFlag().shouldBeTrue()
    }

    @Test
    fun `no pending payment leaves the flag off`() {
        info(mockPurchase("eu.darken.myperm.iap.upgrade.pro")).toPendingFlag().shouldBeFalse()
    ...
    fun `a pending payment is not ownership`() {
        val ownership = pendingInfo(mockPurchase("eu.darken.myperm.iap.upgrade.pro")).toOwnership()
