...
class GplayUpgradeViewModelTest : BaseTest() {
    ...
    fun `restore that finds a purchase emits RestoreSucceeded`() = runTest2(context = testDispatcher) {
        val repo = mockRepo()
        coEvery { repo.restorePurchaseNow() } returns checked(proInfo(mockPurchase("eu.darken.myperm.iap.upgrade.pro")))
        val vm = buildVm(repo)
    ...
    fun `contact support navigates to the guided support form`() = runTest2(context = testDispatcher) {
        ...
        navEvents shouldBe listOf(NavEvent.GoTo(Nav.Settings.ContactForm))
        collector.cancel()
    }
