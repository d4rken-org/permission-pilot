 ...
 * It also has to render the CURATED strings: the dialog used to show the raw exception message.
 */
...
class ComposeErrorDialogTest : BaseTest() {
    ...
    fun `a fixable error offers its action next to a dismiss`() {
        ...
        // The curated troubleshooting body, not the exception's own message: rendering the raw
    ...
    fun `dismissing closes the dialog without launching anything`() {
        ...
        composeRule.onAllNodesWithText(context.getString(R.string.general_close_action)).assertCountEquals(0)
    ...
    fun `an ordinary error keeps the acknowledge-only dialog`() {
        ...
        composeRule.onAllNodesWithText(context.getString(R.string.general_close_action)).assertCountEquals(0)
        composeRule.onAllNodesWithText("Google Play").assertCountEquals(0)
        // Even without a curated error, the dialog titles itself through localized(): the generic
        // error label plus the type, not the platform's bare "Alert".
        composeRule.onNodeWithText(
            "${context.getString(R.string.general_error_label)}: RuntimeException"
        ).assertExists()
    }
    ...
    fun `the unavailable-billing error carries a fix action`() {
        ...
        localized.label shouldBe context.getString(R.string.upgrades_gplay_unavailable_error)
        localized.description shouldBe context.getString(R.string.upgrades_gplay_unavailable_error_description)
